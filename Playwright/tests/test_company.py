...
from utils.data_generator import CompanyData, attach_generated_company_data


def test_add_company(authenticated_page, config):
    dashboard = DashboardPage(authenticated_page)
    company = CompanyPage(authenticated_page)

    generated_data = CompanyData.generate_with_metadata()
    data = generated_data.data
    attach_generated_company_data(generated_data)
    ...
    try:
        ...
        if registration_started:
            company_deleted = company.cleanup_created_company(
                data["company_name"],
                config["base_url"]
            )
            ...
            if company_created:
                assert company_deleted, (
                    "The company created by this test could not be found for cleanup."
                )
